...
namespace algorithms {
	...
	struct SearchNode //info about tiles and successors 
	{
		Tile* tile; 
		bool isFirst; 
		bool isLast; 

		//needs a list of adjacent tiles?
		std::list<Edge*> edges; 
	...
	struct PlannerNode //info about paths, costs, and estimates
	{
		PlannerNode* parent; 
		SearchNode* vertex; //state?
		float heuristicCost;
		float givenCost;
		float finalCost;

		PlannerNode(SearchNode* _ins)
		{
			vertex = _ins; 
			parent = nullptr; 
			heuristicCost = 0.0f; 
	...
	std::unordered_map<Tile*, SearchNode*> SearchMap;
	...
	{
		return (lhs->finalCost > rhs->finalCost); //(lhs->givenCost > rhs->givenCost);//(lhs->heuristicCost > rhs->heuristicCost);
	}

	PriorityQueue<PlannerNode*> open(isGreater);
